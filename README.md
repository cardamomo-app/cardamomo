...
Link to another card by putting its current number in the destination, for example `[See this idea](12)`. Clicking the rendered link expands any folded ancestors, scrolls to that card, and opens it for editing. Cardamomo stores a permanent destination and shows its current number while editing, so moving cards does not break links. Removed destinations show a message instead of opening another card. Save a `.cardamomo` document to preserve these connections across devices. Plain Markdown exports keep internal `#card-…` destinations; other Markdown apps will not resolve them automatically.

Case cycling advances from the selection’s current casing; mixed casing starts at lowercase. Sentence case capitalizes the start of the selection, each sentence, and each paragraph. Word Case keeps common English connecting words (such as “and,” “of,” “the,” and “to”) lowercase, while capitalizing the first and last word of the selection. The shortcut works on the selected Markdown source and supports typing undo.
